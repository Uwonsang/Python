...
def main():
    ...
    #strike가 3일때 까지 while문을 반복해준다. input변수가 0이면 종료한다.
        while StrikeAndBalls[0] < 3:
            user_input_number = input("Input guess number : ")
            if user_input_number == "0":
                endgame = True
                break
    # 조건에 맞는 값들은 strike와 ball을 표시해주고 그렇지 못한 값들은 다시 입력하라는 명령어를 입력한다.
            elif is_validated_number(user_input_number) == True:
                StrikeAndBalls = get_strikes_or_ball(user_input_number, random_number)
